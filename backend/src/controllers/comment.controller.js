...
class CommentController {
    ...
    static async getComments(req, res) {
        ...
        let comments = await CommentModel.find({article: article})
            .populate('user')
            .sort({date: -1});
        //const calculateOffset=comments.length-offset;//Расчёт длины массива
        //let slicedComments = comments.slice(0, calculateOffset);//Отделение старых комментов в кол-ве offset
